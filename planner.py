...
def get_manual_nerf(name):
    if name == "empty":
        ...
        class FakeRenderer:
            @typechecked
            def get_density(
                self, points: TensorType["batch":..., 3]
            ) -> TensorType["batch":...]:
...
class Planner:
    def __init__(self, renderer, start_pose, end_pose, cfg):
        ...
        body = torch.stack(
            torch.meshgrid(
                torch.linspace(
                    -self.x_length / 2, self.x_length / 2, self.cloud_density_per_axis
                ),
                torch.linspace(
                    -self.y_length / 2, self.y_length / 2, self.cloud_density_per_axis
                ),
                torch.linspace(
                    -self.z_length / 2, self.z_length / 2, self.cloud_density_per_axis
                ),
            ),
            dim=-1,
        ).to(device)
    ...
    def plan_traj(self, state_estimate, init_actions, update=0):
        ...
        path_propagation = path(
            init_actions, num_steps, self.mass, self.g, self.I, self.dt
        )
        ...
        for it in range(num_iter):
            ...
            if it % 100 == 0:
                self.save_poses(
                    projected_states.cpu().detach().numpy(),
                    "./paths/drone_pose" + f"{it}.json",
                )
    ...
    def get_density_loss(self, states):
        ...
        points_in_body_frame = torch.cat(
            (
                points_in_body_frame,
                torch.ones((points_in_body_frame.shape[0], 1)).to(device),
            ),
            -1,
        )
        ...
        distances = torch.norm(
            next_states[1:, :3] - prev_states[:-1, :3], dim=1, p=2
        ).to(device)
    ...
    def get_action_loss(self, actions):
        ...
        action_loss = torch.mean(
            self.thrust_penalty * actions[:, 0] ** 2
            + self.torque_penalty * (torch.norm(actions[:, 1:], dim=1, p=2)) ** 2
        )
    ...
    def plot_trajectory(self, states):
        ...
        zcyl = np.linspace(-5, 5, 2)
        thetas, zcyl = np.meshgrid(thetas, zcyl)
        xcyl = math.sqrt(2) * np.sin(thetas)
        ycyl = math.sqrt(2) * (np.cos(thetas) + 1)

        ax = plt.axes(projection="3d")
        ax.plot_surface(xcyl, ycyl, zcyl, color="silver")

        # Plot the planned trajectory
        ax.scatter3D(x, y, z, c="green", label="Current plan")

        # Plot the trajectory so far and end pose
        ax.scatter3D(
            self.trajectory[:, 0],
            self.trajectory[:, 1],
            self.trajectory[:, 2],
            c="red",
            label="Agent's trajectory so far",
        )
        ax.scatter3D(
            self.end_pose[0],
            self.end_pose[1],
...
if __name__ == "__main__":
    ...
    start_pose[:3] = torch.tensor([0.0, -0.8, 0.01])
    end_pose[:3] = torch.tensor([0.0, 3.0, 0.6])
    ...
    init_actions = torch.Tensor(20, 4).uniform_(-1, 1)
    ...
    for iter in range(20):
        ...
        proj_states, action_planner = planner.plan_traj(
            current_state, action_planner, update=True
        )
